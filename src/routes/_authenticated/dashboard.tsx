...
function ReferralFeeCard({ profile, userId }: { profile: Profile; userId: string }) {
  void profile; void userId;
  return (
    <div className="bg-card border border-border rounded-sm p-6 space-y-3">
      <div>
        <h3 className="text-lg font-extrabold tracking-tight">Referral Payout</h3>
        <p className="text-sm text-muted-foreground max-w-2xl mt-1">
          When you send a trip to another transportation provider through My Florida NEMT, the
          referral payout is calculated automatically by the system using the current platform
          referral rate set by My Florida NEMT. It is paid to you after the trip is completed,
          according to the standard payout hold rules.
        </p>
      </div>
      <ul className="text-sm text-muted-foreground list-disc pl-5 space-y-1">
        <li>The referral rate is set and maintained by My Florida NEMT administrators.</li>
        <li>You do not need to enter or adjust anything — nothing is manual.</li>
        <li>The exact referral amount for each trip appears on that trip&apos;s financial breakdown and in your Payouts panel once the trip is completed.</li>
      </ul>
    </div>
  );
}
