...
import { getMyProviderBalance, requestCashout, getFinSettings } from "@/lib/finance/finance.functions";
import { formatCents } from "@/lib/finance/constants";

/**
 * Provider Balance panel — the single money surface for providers.
 * Shows available/pending/lifetime-paid-out, expected payout dates,
 * cash-out button, cash-out history, and full ledger.
 */
export function ProviderBalancePanel() {
  const qc = useQueryClient();
  const load = useServerFn(getMyProviderBalance);
  ...
  const q = useQuery({ queryKey: ["provider-balance"], queryFn: () => load(), refetchInterval: 30_000 });
  ...
  const pending = ledger.filter((l) => l.state === "pending");
  ...
  async function submitCashout() {
    ...
    try {
      ...
      setAmount("");
      await qc.invalidateQueries({ queryKey: ["provider-balance"] });
    } catch (e: unknown) {
    ...
    <section className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-2">
          <label className="text-sm">
            ...
            <input
              ...
              placeholder="0.00"
              className="w-40 border border-input bg-background rounded-sm px-3 py-1.5" />
          </label>
          <button
            type="button"
          ...
          <button
            type="button" disabled={busy || (balance.available_cents ?? 0) < 100}
      ...
      {pending.length > 0 && (
        <div className="border border-border rounded-sm bg-card">
          <div className="px-4 py-2 text-xs font-bold uppercase border-b border-border">Expected payout dates</div>
          <table className="w-full text-sm">
            <thead className="text-[10px] uppercase text-muted-foreground">
              <tr><th className="text-left px-3 py-1">Trip</th><th className="text-left px-3 py-1">Note</th><th className="text-right px-3 py-1">Amount</th><th className="text-left px-3 py-1">Available</th></tr>
            </thead>
            <tbody className="divide-y divide-border">
              {pending.map((p) => (
                <tr key={p.id}>
                  <td className="px-3 py-1 font-mono text-xs">{p.trip_id?.slice(0, 8) ?? "—"}</td>
                  <td className="px-3 py-1 text-xs">{p.note ?? ""}</td>
                  <td className="px-3 py-1 text-right tabular-nums">{formatCents(p.amount_cents)}</td>
                  <td className="px-3 py-1 text-xs">{p.available_at ? new Date(p.available_at).toLocaleString() : "Pending Medicaid receipt"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="border border-border rounded-sm bg-card">
        ...
        {cashouts.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="text-[10px] uppercase text-muted-foreground">
              <tr><th className="text-left px-3 py-1">Requested</th><th className="text-right px-3 py-1">Amount</th><th className="text-left px-3 py-1">Status</th><th className="text-left px-3 py-1">Completed</th></tr>
            </thead>
            <tbody className="divide-y divide-border">
              {cashouts.map((c) => (
                <tr key={c.id}>
      ...
      <div className="border border-border rounded-sm bg-card">
        <div className="px-4 py-2 text-xs font-bold uppercase border-b border-border">Recent ledger</div>
        <table className="w-full text-sm">
          <thead className="text-[10px] uppercase text-muted-foreground">
            <tr><th className="text-left px-3 py-1">When</th><th className="text-left px-3 py-1">Kind</th><th className="text-right px-3 py-1">Amount</th><th className="text-left px-3 py-1">State</th><th className="text-left px-3 py-1">Note</th></tr>
          </thead>
          <tbody className="divide-y divide-border">
            {ledger.map((l) => (
              <tr key={l.id}>
                <td className="px-3 py-1 text-xs">{new Date(l.created_at).toLocaleString()}</td>
                <td className="px-3 py-1 text-xs capitalize">{l.kind}</td>
                <td className={`px-3 py-1 text-right tabular-nums ${l.amount_cents < 0 ? "text-muted-foreground" : ""}`}>{formatCents(l.amount_cents)}</td>
                <td className="px-3 py-1 text-xs capitalize">{l.state}</td>
                <td className="px-3 py-1 text-xs">{l.note ?? ""}</td>
              </tr>
            ))}
          </tbody>
        </table>
