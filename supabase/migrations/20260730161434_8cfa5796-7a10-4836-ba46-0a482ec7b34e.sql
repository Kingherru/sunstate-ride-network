...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Member profile not found for user %', _user_id;
  END IF;
...
REVOKE ALL ON FUNCTION public.admin_set_membership(uuid, public.membership_tier, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_set_membership(uuid, public.membership_tier, text) TO authenticated, service_role;